...
for lang in ('en', 'zh'):
 ...
 def entries(items):
  ...
  for e in items:
   ...
   logo = f'<a class="organization-logo{variant}" href="{esc(e["url"], quote=True)}" target="_blank" rel="noopener noreferrer" aria-label="{local(e["title"])}"><img src="{prefix}{esc(e["logo"])}" alt="{local(e["title"])}{" logo" if lang == "en" else "标志"}" width="64" height="64"></a>'
 ...
 def hobbies():
  ...
  for i, hobby in enumerate(data.get('hobbies', [])):
   ...
   for j, photo in enumerate(hobby.get('images', [])):
    slides.append(f'<figure class="hobby-slide"{ " hidden" if j else ""}><img src="{prefix}{esc(photo["src"], quote=True)}" alt="{local(photo["alt"])}" loading="lazy"></figure>')
   empty = '<div class="hobby-empty"><span aria-hidden="true">＋</span><p>Photos coming soon.</p></div>' if not slides else ''
   panels.append(f'<div class="hobby-panel" role="tabpanel" id="hobby-panel-{key}" aria-labelledby="hobby-tab-{key}" tabindex="0"{ " hidden" if i else ""}><h3>{local(hobby["title"])}</h3><div class="hobby-stage">{empty}{"".join(slides)}<div class="hobby-pagination"><button type="button" data-step="-1" aria-label="Previous photo" disabled>‹</button><span class="hobby-count" role="status" aria-live="polite">{ "1 / " + str(len(slides)) if slides else "0 / 0"}</span><button type="button" data-step="1" aria-label="Next photo" disabled>›</button></div></div></div>')
